...
class VendorOnboardingService extends MedusaService({ VendorApplication, VendorApplicationEvent, VendorApplicationMutation }) {
  ...
  @InjectManager()
  async fenceApproval(operationId: string, update: { member_id?: string; seller_id?: string; created_member?: boolean; complete?: boolean; failed?: boolean }, @MedusaContext() context?: Context<EntityManager>) {
    return this.baseRepository_.transaction(async (manager: EntityManager) => {
      ...
      if (complete) {
        if (!mutation.member_id || !mutation.seller_id) throw new OnboardingError("approval_in_progress");
