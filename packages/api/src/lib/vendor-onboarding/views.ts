...
export async function applicationView(container: MedusaContainer, app: ApplicationRecord, verified = false): Promise<ApplicationView> {
  ...
  const seller = app.seller_id ? (await native.listSellers({ id: app.seller_id }))[0] : null;
  let access = false;
  if (app.member_id && app.seller_id && app.status === "approved") {
    try { await requireVendorAccess(container, app.member_id, app.seller_id); access = true; }
...
export async function applicantResponse(container: MedusaContainer, input: ApplicantIdentity): Promise<ApplicationResponse> {
  ...
  return { application: app ? await applicationView(container, app, live.emailVerified) : null, applicant: { email: live.email, email_verified: live.emailVerified, existing_vendor_access: existingAccess }, unread_count };
}
