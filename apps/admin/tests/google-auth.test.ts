...
test("Google redirects reject untrusted hosts, missing state, and unsafe callbacks", () => {
  ...
  assert.equal(googleCallbackUrl("http://localhost:7000/auth/google/callback"), "http://localhost:7000/auth/google/callback");
...
test("completion refreshes the canonical token and preserves MFA", async () => {
  ...
  assert.deepEqual(requests[0], ["user", "google-admin", { code: "code", state: "state" }]);
  assert.deepEqual(requests[1], { method: "POST", body: { actor_type: "user", existing_token: "existing-session" } });
});
