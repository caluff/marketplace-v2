...
export function OrderFinancePanel({ data }: { data: OrderFinanceResponse }) {
  ...
  const [operation, setOperation] = useState<"refund" | "cancel">(
    data.finance.refund.allowed ? "refund" : "cancel",
  );
  ...
  const eligibility =
    operation === "cancel" ? finance.cancellation : finance.refund;
  const amount =
    operation === "cancel"
          ...
          >
            <div className="space-y-2">
              ...
              <select
                ...
                value={operation}
                onChange={(event) => {
                  setOperation(event.target.value as "refund" | "cancel");
                  setConfirmed(false);
                }}
            ...
            {!eligibility.allowed ? (
              ...
              <>
                ...
                <Button
                  type="submit"
                  variant="outline"
                  className="min-h-11 border-destructive/40 text-destructive"
                >
                  {isPending
                    ? "Procesando…"
                    : operation === "cancel"
